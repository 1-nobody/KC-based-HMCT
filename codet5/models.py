...
from transformers.modeling_t5 import T5ForBiSTM
import logging
...
def build_or_load_gen_model_biSTM(args):
    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_type]
    model_class = T5ForBiSTM
    config = config_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path)
    if args.model_type != 'codet5':
        tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name)
    if args.model_type == 'roberta':
        ...
        model = model_class.from_pretrained('./models/codet5_small')

    if args.model_type == 'codet5':
        # reset special ids: pad_token_id = 0, bos_token_id = 1, eos_token_id = 2
        config, model, tokenizer = load_codet5_biSTM(config, model, tokenizer_class,
                                                     add_lang_ids=args.add_lang_ids,
                                                     tokenizer_path=args.tokenizer_path)
    logger.info("Finish loading model [%s] from %s", get_model_size(model), args.model_name_or_path)
...
def build_or_load_gen_model(args):
    ...
    if args.model_type == 'roberta':
        ...
        # model = model_class.from_pretrained("codet5_base")
        model = model_class.from_pretrained(args.model_name_or_path)
