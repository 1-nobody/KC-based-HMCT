            ...
            if cur_mask_token_id not in im:  # Note The first segment delimiter is incorrect
                ...
                for s1 in range(s, len(im)):
                    if im[s1] == 2 or tokenizer.decode([im[s1]]).startswith('<extra_id_'):
                        return None, mask_tokens[cur_mask_index + 1:], s1  # None,The list of remaining segment separators, the longest of which can be treated as the last position correctly generated
...
def get_forward_inputs_and_type_ids(result, source_id, STM_tokenizer):
    ...
    for j in range(len(result[0])):  # Get the prefix [[Validation segment 1] [Non-validation segment 1] [Validation segment 2] [Non-validation segment 2]
...
def get_BISTM_model(fp, bp):
    parser = argparse.ArgumentParser()
    args = add_args(parser)
    args.trained_model_path = fp
    logger.info(args)
    set_dist(args)
    set_seed(args)
    ...
    if args.sub_task == 'python-java':
        args.trained_model_path = \
            r"./g4g/python2java/bistm/forward_bao/sample_ast/pytorch_model.bin"
    else:
        args.trained_model_path = \
            r"./g4g/java2python/bistm/forward_bao/sample_ast/pytorch_model.bin"
    ...
    if args.sub_task == 'python-java':
        args.trained_model_path = \
            r"./g4g/python2java/bistm/backward/sample_ast/pytorch_model.bin"
    else:
        args.trained_model_path = \
            r"./g4g/java2python/bistm/backward/sample_ast/pytorch_model.bin"
...
def handle_prefix_or_suffix_in_outs_small_range(valid_segment, ast_data, tokenizer, is_prefix=True, p=0.1):
...
def handle_prefix_or_suffix_in_outs_discard(valid_segment, is_prefix=True, p_d=1):
    ...
    for v in valid_segment:
        start_pos = 0
        end_pos = len(v[0])
        random_choice = np.random.choice([True, False], p=[p_d, 1 - p_d])
        # print('random_choice', random_choice)

        if not (random_choice or 1 in v[0] or 2 in v[0]):
            start_pos = end_pos
...
def get_human_segments_2_0(valid_segment, tokenizer, p_d, p):
    origin_false, origin_true = get_origin_false_and_true(valid_segment)
    ast_data = None
    # This is the case where the upper and lower bounds of the verification segment are reduced
    prefix_valid_segment = handle_prefix_or_suffix_in_outs_small_range(valid_segment, ast_data, tokenizer, p=p)
    ...
    #  Simulation of a missing validation segment
    prefix_valid_segment = handle_prefix_or_suffix_in_outs_discard(valid_segment1[0], p_d=p_d)
    suffix_valid_segment = handle_prefix_or_suffix_in_outs_discard(valid_segment1[2], is_prefix=False, p_d=p_d)
...
def get_human_segments(valid_segment):
    ...
    prefix_valid_segment = []
    for v in valid_segment[0]:
        if 1 in v[0] or np.random.choice([True, False], p=[0.6, 0.4]):
            # if 1 in v[0] or True:
            prefix_valid_segment.append(v)
        else:
            prefix_valid_segment[-1][1] += (v[0] + v[1])
    ...
    pre_false = []
    for v in valid_segment[2]:
        if 2 in v[0] or np.random.choice([True, False], p=[0.6, 0.4]):
            # if 2 in v[0] or True:
            suffix_valid_segment.append(
...
def eval_bleu_epoch(args, eval_data, eval_examples, model, tokenizer, split_tag, segment_model, forward_BiSTM,
                    backward_BiSTM, BiSTM_tokenizer, name, start_count):
    ...
    model.eval()
    import csv
    dir_path = os.path.join(args.output_dir, name)
    # ex-config
    top_num = 1

    if not os.path.exists(dir_path):
        os.mkdir(dir_path)
    intermediate_result1 = csv.writer(
        open(os.path.join(dir_path, 'intermediate_results_6.csv'), 'w', encoding='utf-8', newline=''))
    intermediate_compute_result1 = csv.writer(
        open(os.path.join(dir_path, 'intermediate_results_compute_6.csv'), 'w', encoding='utf-8', newline=''))

    intermediate_result2 = csv.writer(
        open(os.path.join(dir_path, 'intermediate_results_6.csv'), 'w', encoding='utf-8', newline=''))
    intermediate_compute_result2 = csv.writer(
        open(os.path.join(dir_path, 'intermediate_results_compute_6.csv'), 'w', encoding='utf-8', newline=''))

    intermediate_result3 = csv.writer(
        open(os.path.join(dir_path, 'intermediate_results_6.csv'), 'w', encoding='utf-8', newline=''))
    intermediate_compute_result3 = csv.writer(
        open(os.path.join(dir_path, 'intermediate_results_compute_6.csv'), 'w', encoding='utf-8', newline=''))
    ...
    intermediate_result7 = csv.writer(
        open(os.path.join(dir_path, 'intermediate_results_7.csv'), 'w', encoding='utf-8', newline=''))
    intermediate_compute_result7 = csv.writer(
        open(os.path.join(dir_path, 'intermediate_results_compute_7.csv'), 'w', encoding='utf-8', newline=''))
    ...
    for count, batch in enumerate(
        ...
        if len(source) >= 509 or len(tgt) >= 509:
            continue
        if len(source) < 100 or len(source) >= 200:
            continue
        with torch.no_grad():
            # [1,-1]
            for p_d in [1, 0.9, 0.8, 0.7, 0.6]:  # -1 indicates that using the Attention cache-based recovery algorithm
                for p in [0.1, 0.4, 0.7]:  # # 'segment_queue' in key means using a non-AC algorithm,
                        ...
                        while tgt_len != -1:
                            ...
                            if isFirst:
                                ...
                                for valid_segments in all_valid_segments:
                                    valid_segment_num_before = len(valid_segments[0]) + len(valid_segments[2]) + 1
                                    # print('before handle:', valid_segment_num_before)
                                    valid_segments = get_human_segments_2_0(
                                        valid_segments, tokenizer=tokenizer, p_d=p_d, p=p
                                    )
                                    valid_segment_num_after = len(valid_segments[0]) + len(valid_segments[2]) + 1
                                    # print('after handle:', valid_segment_num_after)
                                    # print('\n')
                                    decode_valid_segments = dfs_decode(valid_segments, BiSTM_tokenizer)
                                    start = time.time()
                                    success = 1
                                    ...
                                    if im is None:  # Forward generation failed, and the system resumed.
                                        ...
                                        preds = forward_BiSTM.generate(forward_input,
                                                                       attention_mask=forward_input.ne(
                                                                           tokenizer.pad_token_id),
                                                                       use_cache=True,
                                                                       num_beams=args.beam_size,
                                                                       early_stopping=args.task == 'summarize',
                                    ...
                                    candidate_im.append(
                                        [candidate_h_distance, im, valid_segments, end - start,
                                         valid_segment_num_before,
                                         valid_segment_num_after])
                            if isFirst:
                                end = time.time()
            ...
            #     # p<0 based on edit distance mode,p>0 based on en-de attention score mode,p=0, pure same concept
            #     for max_same_length in [0, 1, 2, 3]:
            #         for p in [0]:
            #
            #             source_ids = copy.deepcopy(source_ids1)
            #             tgt = copy.deepcopy(tgt1)
            ...
            #                 sum = 0  # The total number of correct fragments
            ...
            #                     [count, prefix, im_res, eval_examples[count].target, h_distance, end - start, view_len,
        ...
        #         # 'segment_queue' in key means using a non-AC algorithm,
        #         # p<0 based on edit distance mode,p>0 based on en-de attention score mode,p=0, pure same concept
        #         for max_same_length in [-1, 1, 2, 3]:
        #             for p in [0]:
        #
        #                 print(max_same_length)
        #
        #                 source_ids = copy.deepcopy(source_ids1)
        #                 tgt = copy.deepcopy(tgt1)
        #                 num_return_sequences = args.beam_size
        ...
        # tgt = tgt1
        # source = source1
        # if len(source) >= 509 or len(tgt) >= 509 or count == 4:
        #     continue
        # #
        # with torch.no_grad():
        #     #
        ...
        #         start = time.time()
        #         prefix = tokenizer.decode(tgt[:tgt_len + 1])
        #         preds, past, _, _ = model.generate(source_ids,
        #                                            attention_mask=source_mask,
        #                                            use_cache=True,
        #                                            num_beams=args.beam_size,
        #                                            early_stopping=args.task == 'summarize',
        ...
        #         intermediate_result1.writerow(
        #             [count, prefix, im_res, eval_examples[count].target, h_distance, end - start, 0])
        #         if tgt_len == -1 or tgt_len >= len(tgt) or len(im) >= 509:
        #             intermediate_compute_result1.writerow(
        #                 [count, eval_examples[count].source, eval_examples[count].target, first_instance, cur_num, 0])
        #             break
        #         print('\n')
        #         print('now: epoch ' + str(count))
        #         # print(tokenizer.decode(tgt[tgt_len]))
        #         print('tgt_len: ' + str(tgt_len))
        #         print('h_distance: ' + str(h_distance))
        #         print('\n\n')
        #         cur_num += 1
        #
            # HMPO-SS
        # with torch.no_grad():
        #     for pattern in range(2, 3):
        #         for classfier in range(3):
        #             num_return_sequences = args.beam_size
        #             past = None
        #             cur_num = 0
        #             preds = []
        ...
        #                 start = time.time()
        #                 prefix = tokenizer.decode(tgt[:tgt_len + 1])
        #                 # The past that is returned at this point may only be a local state corresponding to a prefix
        #                 if isFirst:
        #                     # Need to get the first generated sequence
        ...
        #                                                                                   prefix=tgt,
        #                                                                                   prefix_len=tgt_len + 1,
        #                                                                                   # The length of the prefix that the user actually authenticates
        #                                                                                   tokenizer=tokenizer,
        #                                                                                   least_past=None,
        ...
        #                                                                       num_return_sequences=num_return_sequences,
        #                                                                       prefix=tgt,
        #                                                                       prefix_len=tgt_len + 1,   # The length of the prefix that the user actually authenticates
        #                                                                       tokenizer=tokenizer,
        #                                                                       least_past=past,  # Some actual local state, less than or equal to the prefix length -1
        #                                                                       is_past=True,
        #                                                                       is_concent_sufix=True,
        #                                                                       sequence_outputs=sequence_outputs,
        #                                                                       first_output=first_output,
        ...
        #             for index, j in enumerate(i):
        #                 if index in [0, 1]:
        #                     cur_tuple.append(j[:, :, :min(tgt_len + 1, j.shape[2]), :])  # The value of the possible state is less than the length of the prefix -1
        #                 else:
        #                     cur_tuple.append(j)
        #             res_tuple.append(tuple(cur_tuple))
        #         past = tuple([past[0], tuple(res_tuple)])
        ...
        #                 token_dic[token].append((c, sequence_output))
        #     im_res = tokenizer.decode(im, clean_up_tokenization_spaces=False)
        #     intermediate_result3.writerow(
        #         [count, prefix, im_res, eval_examples[count].target, h_distance, end - start,
        #          best_suffix, has_contains_false])
        #     if tgt_len == -1 or tgt_len >= len(tgt) or len(im) >= 509:
        #         intermediate_compute_result3.writerow(
        #             [count, eval_examples[count].source, eval_examples[count].target, first_instance,
        #              cur_num])
        #         break
        #     print('\n')
        ...
        # HMPO-ACSS-classifier
        # pattern:[reverse,skip-5,skip-10,skip-15,skip-20]->[0,1,2,3,4]
...
def main():
    ...
    if args.do_test:
        ...
        args.sub_task = 'python-java'

        if args.sub_task == 'python-java':
            trained_model_path = r'./g4g/python2java/paraphrase/checkpoint-best-ppl/GTM/pytorch_model.bin'

            sample_forward_bistm_path = r'./g4g/python2java/bistm/forward_bao/sample_ast/pytorch_model.bin'
            sample_backward_bistm_path = r'./g4g/python2java/bistm/backward/sample_ast/pytorch_model.bin'

            forward_bistm_path = r'./g4g/python2java/bistm/forward_bao/pytorch_model.bin'
            backward_bistm_path = r'./g4g/python2java/bistm/backward/pytorch_model.bin'
        else:
            trained_model_path = r'./g4g/java2python/paraphrase/checkpoint-best-ppl/GTM/pytorch_model.bin'
            forward_bistm_path = r'./g4g/java2python/bistm/forward_bao/pytorch_model.bin'
            backward_bistm_path = r'./g4g/java2python/bistm/backward/pytorch_model.bin'
            sample_forward_bistm_path = r'./g4g/java2python/bistm/sample/forward_bao/pytorch_model.bin'
            sample_backward_bistm_path = r'./g4g/java2python/bistm/sample/backward/pytorch_model.bin'
        if os.path.isfile(trained_model_path):
            logger.info("Reload model from {}".format(trained_model_path))
            model.load_state_dict(torch.load(trained_model_path))

            forward_model, backward_model, bistm_tokenizer = get_BISTM_model(forward_bistm_path,
                                                                             backward_bistm_path)  # BISTM forward translation model
            sample_forward_model, sample_backward_model, _ = get_BISTM_model(sample_forward_bistm_path,
                                                                             sample_backward_bistm_path)  # BISTM backward translation model

            eval_examples, eval_data = load_and_cache_gen_data(
                args, 'g4g_functions', pool, tokenizer, 'test', only_src=True, is_sample=False
            )
            is_sample = False
            eval_bleu_epoch(args, eval_data, eval_examples, model, tokenizer, 'test', None, forward_model,
                            backward_model, bistm_tokenizer, 'g4g' + ('_sample_' if is_sample else '_') + args.sub_task,
                            0)
            # is_sample = True
            # eval_bleu_epoch(args, eval_data, eval_examples, model, tokenizer, 'test', None, sample_forward_model,
            #                 sample_backward_model, bistm_tokenizer,
            #                 'g4g' + ('_sample_' if is_sample else '_') + args.sub_task, 0)

            eval_examples, eval_data = load_and_cache_gen_data(
                args, 'transcoder_test_gfg', pool, tokenizer, 'test', only_src=True, is_sample=False
            )
            is_sample = False
            eval_bleu_epoch(args, eval_data, eval_examples, model, tokenizer, 'test', None, forward_model,
                            backward_model, bistm_tokenizer,
                            'transcoder' + ('_sample_' if is_sample else '_') + args.sub_task, 0)
            # is_sample = True
            # eval_bleu_epoch(args, eval_data, eval_examples, model, tokenizer, 'test', None, sample_forward_model,
            #                 sample_backward_model, bistm_tokenizer,
            #                 'transcoder' + ('_sample_' if is_sample else '_') + args.sub_task, 0)
