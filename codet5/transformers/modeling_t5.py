...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    Examples::
        ...
        if "BiSTM_type_ids" in kwargs:
            inputs_embeds = self.get_input_embeddings()(input_ids) + self.get_input_embeddings()(
                kwargs["BiSTM_type_ids"])
        ...
        return decoder_outputs + encoder_outputs + (sequence_output,)
...
@add_start_docstrings("""T5 Model with a `language modeling` head on top. """, T5_START_DOCSTRING)
class T5ForBiSTM(T5PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.model_dim = config.d_model
        ...
        #self.token_emb_and_type_emb_linear = nn.Linear(config.d_model*2, config.d_model, bias=False)
    ...
    Examples::
        ...
        # token_emb = self.get_input_embeddings()(input_ids)
        #
        # type_emb = self.get_input_embeddings()(BiSTM_type_ids)
        #
        # inputs_embeds = token_emb+type_emb
        ...
        if labels is not None:
            ...
            loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), labels.view(-1))
            # TODO(thom): Add z_loss https://github.com/tensorflow/mesh/blob/fa19d69eafc9a482aff0b59ddd96b025c0cb207d/mesh_tensorflow/layers.py#L666
            decoder_outputs = (loss,) + decoder_outputs+ (lm_logits.view(-1, lm_logits.size(-1)).argmax(-1),) + (
                labels.view(-1),)
